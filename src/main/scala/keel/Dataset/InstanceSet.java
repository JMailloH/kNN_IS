 ...
 * formated data file. It provides a set of methods that permit to get each
 * instance, get the whole set of instances, get the number of instances, etc.
 *
...
public class InstanceSet {
	...
	/**
	 * Creates a new InstanceSet with the header and Instances from the passed
	 * object It performs a deep (new allocated) copy.
	 * 
	 * @param is
	 ...
	 *
	 * It stores the static-defined attributes in the class Attributes as non
	 * static in the object attributes. After this it does not remove the
	 ...
	 * @param isTrain
	 *            is a flag that indicate if the database is for a train or for
	 *            a test.
	 * @throws DatasetException
	 *             if there is any semantical error in the input file.
	 * @throws HeaderFormatException
	 *             if there is any lexical or sintactical error in the header of
	 *             the input file
	 */
	public void readSet(String fileName, boolean isTrain)
		...
		// Reading information in the header, i.e., @relation, @attribute,
		// @inputs and @outputs
		parseHeader(parser, isTrain);

		System.out.println(" The number of output attributes is: "
	  ...
	  /**
	   * This function is created for hadoop. 
	   */
	
	public void parseHeaderFromString(ArrayList<String> cabecera, boolean isTrain){
	...
	/**
	 * It reads the information in the header of the file. It reads relation's
	 * name, attributes' names, and inputs and outputs.
	 *
	 * @param parser
	 *            is the parser of the data set
	 * @param isTrain
	 *            is a boolean indicating if this is a train set (and so
	 *            parameters information must be read) or a test set (parameters
	 *            information has not to be read).
	 */
	...
	public void parseHeader(InstanceParser parser, boolean isTrain) {
		...
		while (!(line = parser.getLine().trim()).equalsIgnoreCase("@data")) {
			...
			if (line.toLowerCase().indexOf("@relation") != -1) {
				if (isTrain)
					Attributes
							.setRelationName(line.replaceAll("@relation", ""));
			}
			...
			if (line.toLowerCase().indexOf("@inputs") != -1) {
				...
				if (isTrain)
					insertInputOutput(aux, lineCount, inputAttrNames, "inputs",
							isTrain);
			...
			if (line.toLowerCase().indexOf("@outputs") != -1) {
				...
				aux = line.substring(8);
				if (isTrain)
					insertInputOutput(aux, lineCount, outputAttrNames,
							"outputs", isTrain);

				System.out.println(" >> Size of the output is: "
						+ outputAttrNames.size());
			}
			header += line + "\n";
		...
		processInputsAndOutputs(isTrain, inputsDef, outputsDef,
	...
	void insertAttribute(String line) {
		...
		if (!st.hasMoreTokens()) { // Parsing a nominal attribute with no
									// definition of values
			// System.out.println
			// ("    > Parsing nominal attribute without values ");
			at.setType(Attribute.NOMINAL);
		} else if (line.indexOf("{") != -1) { // Parsing a nominal attribute
			// System.out.println
			// ("    > Parsing nominal attribute with values: "+line );
			at.setType(Attribute.NOMINAL);
			...
			// System.out.println ( "      > The Nominal values are: " +
			// line.substring( indexL+1, indexR) );
			StringTokenizer st2 = new StringTokenizer(line.substring(
					indexL + 1, indexR), ",");
		...
		while (st.hasMoreTokens()) {
			...
			if (Attributes.getAttribute(attName) == null) {
				...
				ErrorInfo er = new ErrorInfo(
						...
						0,
						isTrain,
						("The attribute " + attName + " defined in @" + type + " in test, it has not been defined in @inputs in its train dataset. It will be ignored"));
				InstanceSet.errorLogger.setError(er);
			} else {
				System.out.println("   > " + type + " attribute considered: "
						+ attName + ".");
	...
	void processInputsAndOutputs(boolean isTrain, boolean inputsDef,
			boolean outputsDef, Vector outputAttrNames, Vector inputAttrNames) {
		...
		if (isTrain) {
			if (!inputsDef && !outputsDef) {
				outputAttrNames.add(Attributes.getAttribute(
						Attributes.getNumAttributes() - 1).getName());
				inputAttrNames = Attributes
						.getAttributesExcept(outputAttrNames);
				outputInfered = true;
			} else if (!inputsDef && outputsDef) {
				inputAttrNames = Attributes
						.getAttributesExcept(outputAttrNames);
			} else if (inputsDef && !outputsDef) {
				outputAttrNames = Attributes
						.getAttributesExcept(inputAttrNames);
				outputInfered = true;
			}

			Attributes
					.setOutputInputAttributes(inputAttrNames, outputAttrNames);
		}
	...
	public Instance getInstance(int whichInstance) {
		if (whichInstance < 0 || whichInstance >= instanceSet.length)
			return null;
	 ...
	 * @param whichAttr
	 *            is the position of the input attribute.
	 * @return a String with the numeric value.
	 * @throws ArrayIndexOutOfBoundsException
	 *             If the index is out of the instance set size.
	 */
	public double getInputNumericValue(int whichInst, int whichAttr)
			throws ArrayIndexOutOfBoundsException {
	 ...
	 *            is the position of the instance.
	 * @param whichAttr
	 *            is the position of the output attribute.
	 * @return a String with the numeric value.
	 * @throws ArrayIndexOutOfBoundsException
	 *             If the index is out of the instance set size.
	 */
	public double getOutputNumericValue(int whichInst, int whichAttr)
			throws ArrayIndexOutOfBoundsException {
		if (whichInst < 0 || whichInst >= instanceSet.length)
			throw new ArrayIndexOutOfBoundsException(
					"You are trying to access to " + whichInst
	 ...
	 *             If the index is out of the instance set size.
	 */
	public String getInputNominalValue(int whichInst, int whichAttr)
			throws ArrayIndexOutOfBoundsException {
		if (whichInst < 0 || whichInst >= instanceSet.length)
			throw new ArrayIndexOutOfBoundsException(
					"You are trying to access to " + whichInst
	 ...
	 *             If the index is out of the instance set size.
	 */
	public String getOutputNominalValue(int whichInst, int whichAttr)
			throws ArrayIndexOutOfBoundsException {
		if (whichInst < 0 || whichInst >= instanceSet.length)
			throw new ArrayIndexOutOfBoundsException(
					"You are trying to access to " + whichInst
	 ...
	 * 
	 * @param instNum
	 *            is the instance removed from the instanceSet.
	 */
	public void removeInstance(int instNum) {
		if (instNum < 0 || instNum >= instanceSet.length)
			return;
	 ...
	 *            is the test set.
	 * @param inputAtt
	 *            is a boolean that is true when the attribute that is wanted to
	 *            be removed is an input attribute.
	 * @param whichAtt
	 *            is a integer that indicate the position of the attriubte to be
	 *            deleted.
	 * @return a boolean indicating if the attribute has been deleted
	 */
	public boolean removeAttribute(InstanceSet tSet, boolean inputAtt,
			int whichAtt) {
		...
		if (storeAttributesAsNonStatic && attributes != null) {
			...
			if (!attributes.removeAttribute(inputAtt, whichAtt)
					|| (tSet != null && !tSet.attributes.removeAttribute(
							inputAtt, whichAtt)))
				return false;
		} else {
			if (!Attributes.removeAttribute(inputAtt, whichAtt))
		...
		for (int i = 0; i < instanceSet.length; i++) {
			if (storeAttributesAsNonStatic && attributes != null) {
				instanceSet[i].removeAttribute(attributes, attToDel, inputAtt,
						whichAtt);
			} else {
		...
		if (tSet != null)
			for (int i = 0; i < tSet.instanceSet.length; i++) {
				if (storeAttributesAsNonStatic && attributes != null)
					tSet.instanceSet[i].removeAttribute(attributes, attToDel,
							inputAtt, whichAtt);
				else
	...
	/**
	 * It does return a new header (not necessary the same header as the input
	 * file one). It only includes the valid attributes, those ones defined in @inputs
	 * and @outputs (or taken as that role following the keel format
	 * specification).
	 * 
	...
	/**
	 * It does return the original header definiton but without @input and @output
	 * in there
	 */
	 ...
	 * 
	 * @param out
	 *            is the PrintWriter where to print
	 */
	...
	/**
	 * It prints the dataset to the specified PrintWriter. The order of the
	 * attributes is the same as in the original file
	 * 
	 * @param out
	 *            is the PrintWriter where to print
	 * @param printInOut
	 *            indicates if the @inputs (1), @outputs(2), both of them (3) or
	 *            any (0) has to be printed
	 */
	...
	/**
	 * Clear the non-Static attributes. The static class Attributes is not
	 * modified.
	 */
	...
	/**
	 * Appends the given attribute to the non-static list of the current
	 * InstanceSet
	 * 
	 * @param at
