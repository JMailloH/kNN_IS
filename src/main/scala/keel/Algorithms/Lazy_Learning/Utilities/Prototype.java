...
public class Prototype
{
     ...
     * @param att InstanceAttributes object.
     */
    public static void setAttributesTypes(Attribute att[])
    {
    ...
    public Prototype(double[] inputs, double[] outputs)
    {
        ...
        this.index = UNKNOW_INDEX;
    	missing = new boolean[inputs.length];
        Arrays.fill(missing, false);
    }
    ...
    public void normalize(){
        ...
        for(int i=0; i<size; ++i)
        {
            ...
            /*
            if(inst.missing(i)){
            	System.out.println("ESTOY PERDIDO,  "+i);
            	this.missing[i] = true;
            }*/
    ...
    public Prototype(Prototype original)
    {
        ...
        this.missing = Arrays.copyOf(original.missing, original.missing.length);
        //this.index = UNKNOW_INDEX;
    ...
    /*
     * Creates a prototype from an input vector that has the class as the last element
     */
    public Prototype(double instancia[]){
        this.inputs = Arrays.copyOf(instancia, instancia.length-1);
        this.outputs = new double[1];
        outputs[0]=instancia[instancia.length-1];
        this.index = UNKNOW_INDEX;
    	missing = new boolean[inputs.length];
        Arrays.fill(missing, false);
        this.normalize();
...
//              System.out.println(comma[1]);
    ...
    public String getInputAsNominal(int i)
    {
        ...
        if(normalized)
        {
           ...
           // System.out.println("Normalizado: "+maxInput_i+ ", "+inputs[i]);
        }
        else
        {
            
        	//System.out.println("No estaba normalizado, "+inputs[i]);
        	//Como ya está desnormalizado, sólo extraemos el índice del atributo nominal
            indexOfNominalAttr = (int)(Math.round(inputs[i]));
        }
        
        //System.out.println("indexOfNominalAttr "+indexOfNominalAttr);
        //System.out.println("Attributes: "+Attributes.getInputAttribute(i));
    ...
    */
    ...
    public Prototype formatear(){
        ...
        for(int i=0; i<nInputs; i++)
        {
           ...
           // System.out.println("min_inputsI[i] "+i+": "+min_inputsI[i]+", "+"max_inputsI[i] "+ max_inputsI[i]);
           }
    	...
    	for (int i=0; i< this.inputs.length; i++){
    		if(type[i] == NOMINAL){
    			...
    			//System.out.println(this.inputs[i]+" ,  "+formateado.inputs[i]);
    		}
    		if(type[i] == INTEGER){
...
/*
	...
	public void readFields(DataInput out) throws IOException {
		...
		for(int i=0; i< this.inputs.length; i++){
			this.inputs[i]= out.readDouble();
		}
		
		outputs[0] = out.readDouble();
	...
	public void write(DataOutput out) throws IOException {
		...
		//out.writeInt(this.numberOfInputs());
		for(int i=0; i< this.numberOfInputs(); i++){
			out.writeDouble(this.inputs[i]);
		}
		
		out.writeDouble(outputs[0]);		
